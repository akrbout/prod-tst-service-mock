...
if [ "$APP_TYPE" = "cian" ]; then
  ...
  exec poetry run uvicorn cian_mock_service.main:app --host 0.0.0.0 --port 80
elif [ "$APP_TYPE" = "hh" ]; then
  echo "Will run hh_mock_service application"
  exec poetry run uvicorn hh_mock_service.main:app --host 0.0.0.0 --port 80
elif [ "$APP_TYPE" = "tk" ]; then
  echo "Will run tk_mock_service application"
  exec poetry run uvicorn tk_mock_service.main:app --host 0.0.0.0 --port 80
else
